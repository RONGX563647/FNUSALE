...
export const productApi = {
  ...
  search(keyword: string, params: PageParams): Promise<Result<PageResult<ProductVO>>> {
    return http.get(`${BASE_URL}/search`, { params: { keyword, ...params } })
  },
  ...
  recognizeCategory(imageUrl: string): Promise<Result<unknown>> {
    return http.post(`${BASE_URL}/ai-category`, null, { params: { imageUrl } })
  },
  ...
  getNearby(
    ...
    return http.get(`${BASE_URL}/nearby`, { params: { longitude, latitude, distance, ...params } })
  },
...
export const categoryApi = {
  ...
  updateStatus(id: number, status: number): Promise<Result<void>> {
    return http.put(`/category/${id}/status`, null, { params: { status } })
  },
