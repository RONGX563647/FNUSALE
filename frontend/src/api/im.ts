...
export const sessionApi = {
  ...
  create(targetUserId: number, productId: number): Promise<Result<number>> {
    return http.post('/session/create', null, { params: { targetUserId, productId } })
  },

  // 获取或创建会话
  getOrCreate(targetUserId: number, productId: number): Promise<Result<number>> {
    return http.get('/session/get-or-create', { params: { targetUserId, productId } })
  },
...
export const messageApi = {
  ...
  sendText(sessionId: number, content: string): Promise<Result<void>> {
    return http.post('/message/text', null, { params: { sessionId, content } })
  },

  // 发送图片消息
  sendImage(sessionId: number, imageUrl: string): Promise<Result<void>> {
    return http.post('/message/image', null, { params: { sessionId, imageUrl } })
  },

  // 发送语音消息
  sendVoice(sessionId: number, voiceUrl: string, duration: number): Promise<Result<void>> {
    return http.post('/message/voice', null, { params: { sessionId, voiceUrl, duration } })
  },
  ...
  addQuickReply(content: string): Promise<Result<void>> {
    return http.post('/message/quick-reply', null, { params: { content } })
  },
  ...
  search(sessionId: number, keyword: string): Promise<Result<unknown>> {
    return http.get('/message/search', { params: { sessionId, keyword } })
  }
}